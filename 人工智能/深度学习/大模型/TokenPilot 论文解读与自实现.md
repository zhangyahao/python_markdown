...
本文提供的自实现 Demo 完整复现了这一思想，代码清晰、可扩展，可作为集成到真实项目中的起点。后续可进一步改进：

- 引入小模型（如 Qwen3.5-35B）作为效用评估器，替代简单的固定规则。
- 实现真正的恢复工具，让 Agent 能通过 `[FULL_ID]` 请求完整内容。
- 支持自适应驱逐间隔，根据上下文增长速度动态调整。
